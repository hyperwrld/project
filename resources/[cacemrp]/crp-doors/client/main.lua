...
Citizen.CreateThread(function()
	...
	for doorId, door in ipairs(doors) do
		if door and not IsDoorRegisteredWithSystem(doorId) then
			...
			points[doorId] = door.zone
		end
	end

	exports['crp-lib']:createBoxZones(points, 'doorZone', GetCurrentResourceName(), false)
end)

AddEventHandler('crp-doors:onPlayerInOut', function(isPointInside, zone)
	if isPointInside then
		local doorState = DoorSystemGetDoorState(zone.id) ~= 0 and true or false

		currentDoorId = zone.id

		ListenForKeys(zone.id)
...
function toggleDoorState(doorId)
	if doors and doors[doorId] then
		if not hasPermission(doorId) then
			return
		end
...
function hasPermission(doorId)
	local currentJob = exports['crp-jobs']:getJob()

	for k, jobName in pairs(doors[doorId].authorizedJobs) do
