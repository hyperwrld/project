...
function CRP.DB:FetchCharacters(identifier, callback)
    if not identifier or identifier == '' then callback:resolve(false) return end
...
function CRP.DB:DoesCharacterExist(firstname, lastname, callback)
    if not firstname or type(firstname) ~= 'string' then callback(false) return end
    if not lastname or type(lastname) ~= 'string' then callback(false) return end
    ...
    CRP.DB:DoesCharacterExist(data.firstname, data.lastname, function(status)
        if status then
            ...
            exports.ghmattimysql:execute(query, parameters, function(result)
                ...
                local query = [[SELECT id, firstname, lastname, gender, dob, job, bank, money, story, phone FROM users WHERE identifier = @identifier AND firstname = @firstname AND lastname = @lastname;]]
                local parameters = {
                    ['@identifier'] = identifier,
                    ['@firstname'] = data.firstname,
                    ['@lastname'] = data.lastname,
                ...
                exports.ghmattimysql:execute(query, parameters, function(result)
                    callback:resolve({ status = true, characterData = result[1] })
                end)
            end)
        end
    end)
...
function CRP.DB:DeleteCharacter(identifier, characterId, callback)
    if not identifier or identifier == '' then callback:resolve({ status = false }) return end
    if not characterId or type(characterId) ~= 'number' then callback:resolve({ status = false }) return end
    ...
    exports.ghmattimysql:execute(query, parameters, function(result)
        if not result.changedRows then callback:resolve({ status = false }) return end

        callback:resolve({ status = true })
    end)
end

function CRP.DB:RetrieveCharacter(identifier, data, callback)
    if not identifier or identifier == '' then callback:resolve(false) return end
    if not data.characterId or type(data.characterId) ~= 'number' then callback:resolve(false) return end
