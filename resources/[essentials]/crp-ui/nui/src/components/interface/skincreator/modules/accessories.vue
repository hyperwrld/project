		...
		methods: {
			modifyAccessories: function(index, isMain, isProp) {
				let accessory = this.accessories.find(
					(element) => element.id == index && element.isProp == isProp
				);

				send('modifyAccessories', {
					index: Number(index),
					value: Number(accessory.value),
					secondValue: Number(accessory.subOptions[0].value),
					isProp: isProp,
				}).then((data) => {
					if (isMain) {
						(accessory.subOptions[0].maxValue = data),
							(accessory.subOptions[0].value = 0);
					}
				});
			},
		},
				...
				let data = (
					<div class='container'>
						<optionInput
							...
							click={this.modifyAccessories}
						/>
						<optionInput
							data={this.accessories[i].subOptions[0]}
							click={this.modifyAccessories}
						/>
					</div>
				);
