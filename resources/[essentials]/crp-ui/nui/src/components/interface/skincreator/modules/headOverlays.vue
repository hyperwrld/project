	...
	export default {
		...
		methods: {
			modifyHeadOverlay: function(index) {
				const arrayIndex = this.headOverlays.findIndex(
						(element) => element.id == index
					),
					data = this.headOverlays;

				send('modifyHeadOverlay', {
					index: index,
					value: data[arrayIndex].value,
					opacity: data[arrayIndex + 1].value,
				});
			},
		},
		render() {
			let options = [],
				headOverlays = this.headOverlays;
			...
			for (let i = 0; i < headOverlays.length; i += 2) {
				let data = (
					<div class='container'>
						<optionInput
							data={headOverlays[i]}
							click={this.modifyHeadOverlay}
						/>
						<optionRange
							data={headOverlays[i + 1]}
							click={this.modifyHeadOverlay}
						/>
					</div>
				);
