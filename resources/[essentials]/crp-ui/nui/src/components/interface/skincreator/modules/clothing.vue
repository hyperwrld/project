	...
	export default {
		...
		methods: {
			modifyClothing: function(index, isMain) {
				...
				send('modifyClothing', {
					index: Number(index),
					value: Number(clothing.value),
					secondValue: Number(clothing.subOptions[0].value),
				}).then((data) => {
					if (isMain) {
						(clothing.subOptions[0].maxValue = data),
							(clothing.subOptions[0].value = 0);
					}
				});
			},
		},
			...
			for (let i = 0; i < this.clothing.length; i++) {
				let data = (
					<div class='container'>
						...
						<optionInput
							data={this.clothing[i].subOptions[0]}
							click={this.modifyClothing}
						/>
					</div>
				);
