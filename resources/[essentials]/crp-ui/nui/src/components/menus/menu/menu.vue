<script>
	...
	import { faGlobeEurope, faLink, faShieldAlt, faTheaterMasks, faTimes, faWalking } from '@fortawesome/free-solid-svg-icons';

	library.add(faTimes, faGlobeEurope, faShieldAlt, faWalking, faTheaterMasks, faLink);
	...
	export default {
		...
		methods: {
			createMenuLayer: function(items, level) {
				let info = {
					inner: true, outer: false, level: level, levelItems: items, sectors: []
				};
									...
									{ menu.sectors.map((sector, index) => {
										return (
											...
											>
												...
												<use href={ '#' + sector.id } x={ sector.centerX } y={ sector.centerY } transform={ sector.useTransform } width='10' height='10' fill='white'/>
											</g>
										)
									})}
